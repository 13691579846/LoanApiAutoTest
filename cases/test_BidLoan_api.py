...
class TestBidLoanApi(Base):
    ...
    add.add_loan_api(method='post',
                     ...
                     data={"memberId": str(do_user('Invest', 'memberid')), "title": "超哥专属标的", "amount": 1000000,
                           "loanRate": 10,"loanTerm": 3, "loanDateType": 0,
                           "repaymemtWay": 4, "biddingDays": 5}
                     )


    sql = 'SELECT Id FROM loan WHERE MemberID=' + str(do_user('Invest', 'memberid')) + ' ' + 'ORDER BY CreateTime DESC LIMIT 1;'
    loan_id = str(mysql(sql=sql)['Id'])
